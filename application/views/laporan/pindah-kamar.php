<!DOCTYPE html>
<html lang="en">

<head>
    <meta charset="UTF-8">
    <title><?= $title ?></title>
    <link rel="stylesheet" href="<?= base_url('assets/css/') ?>table.css" media="all" />
</head>

<body>
    <header>
        <div style="text-align: center; font-weight: bold;">
            <img src="<?= base_url('assets/img/slider/kabupaten.png') ?>" style="height: 110px" ; width="auto">
        </div>
        <br>
        <div style="text-align:center;">
            <div>ASRAMA MAHASISWA</div>
            <div>PEMERINTAH KABUPATEN KAPUAS</div>
            <div>KALIMANTAN TENGAH</div>
            <div style="font-size: 10px;">Jalan S.Parman Gg Kalimantan II Banjarmasin Email:asramakapuas14@gmail.com</div>
        </div>
        <header class="clearfix">
            <h1>Laporan Persetujuan Pindah Kamar</h1>
            <!-- <div style="text-align:Right;">
                <div>Banjarmasin,<?= date('d F Y'); ?> </div>
            </div> -->
        </header>
        <main>
            <div class="table3">
                <table style="width: 623px;">
                    <tbody>
                        <tr style="height: 25px;">
                            <td style="width: 171px; height: 25px;">Nama Penghuni</td>
                            <td style="width: 17px; height: 25px;">:</td>
                            <td style="width: 434px; height: 25px;">&nbsp;<?= $penghuni['name_penghuni']; ?></td>
                            <td style="width: 277px; height: 25px; text-align: right;">Banjarmasin,<?= date('d-m-Y'); ?> </td>
                        </tr>
                        <tr style="height: 25px;">
                            <td style="width: 171px; height: 25px;">Email Penghuni</td>
                            <td style="width: 17px; height: 25px;">:</td>
                            <td style="width: 434px; height: 25px;">&nbsp;<?= $penghuni['email_pindah']; ?></td>
                            <td style="width: 277px; height: 25px;">&nbsp;</td>
                        </tr>
                        <tr style="height: 25px;">
                            <td style="width: 171px; height: 25px;">Alasan Pindah</td>
                            <td style="width: 17px; height: 25px;">:</td>
                            <td style="width: 434px; height: 25px;">&nbsp;<?= $penghuni['alasan']; ?></td>
                            <td style="width: 277px; height: 25px;">&nbsp;</td>
                        </tr>
                    </tbody>
                </table>
            </div>
            <!-- <table>
                <tr>
                    <td width="100" height="25">Nama Penghuni</td>
                    <td width="5" height="25">:</td>
                    <td><?= $penghuni['name_penghuni']; ?></td>
                </tr>
                <tr>
                    <td width="100" height="25">Email Penghuni</td>
                    <td width="5" height="25">:</td>
                    <td><?= $penghuni['email_pindah']; ?></td>
                </tr>
                <tr>
                    <td width="100" height="25">Alasan Pindah</td>
                    <td width="5" height="25">:</td>
                    <td><?= $penghuni['alasan']; ?> </td>
                </tr>
            </table> -->
            <br>
            <div class="table1">
                <table>
                    <thead>
                        <tr>
                            <th>Kamar Asal</th>
                            <th>Kamar Tujuan</th>
                            <th>Tanggal Pengajuan</th>
                        </tr>
                    </thead>
                    <tbody>

                        <tr>
                            <td><?= $penghuni['no_kamar_sekarang']; ?></td>
                            <td><?= $penghuni['no_kamar_baru']; ?></td>
                            <td><?= date('d-m-Y', $penghuni['tgl_pindah']); ?></td>

                        </tr>
                    </tbody>
                </table>
            </div>
            <br>
            <br>
            <br>
            <div style="text-align:Right; font-weight: bold;">
                <div>Mengetahui</div>
                <br>
                <br>
                <br>
                <br>
                <div><?= $user['name']; ?></div>
            </div>
</body>

</html>