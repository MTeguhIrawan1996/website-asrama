...
class Auth extends CI_Controller
{
	...
	public function registration()
	{
		...
		if ($this->form_validation->run() == false) {
			...
			$this->session->set_flashdata('message', '<div class="alert alert-success" role="alert">Congretulation! your account has been created Please activate your account!!</div>');
			redirect('auth');
		}
	}
	...
	private function _sendEmail($token, $type)
	{
		...
		// 	$mail->setFrom('asramakapuas14@gmail.com', 'Asrama Mahasiswa Kapuas');
		...
		// 		$$mail->Body = 'Click this link to reset your account : <a href="' . base_url() . 'auth/resetpassword?email=' . $this->input->post('email') . '&token=' . urlencode($token) . '">Reset Password</a>';
		// 	}
		...
		$this->email->from('asramakapuas14@gmail.com', 'Asrama Mahasiswa Kapuas');
	...
	public function verify()
	{
		...
		//ambil data user didatabase
		$user = $this->db->get_where('user', ['email' => $email])->row_array();

		//cek email apakah sama dengan email di tabel user
		if ($user) {

			//cek apakah token diurl sama dengan di tabel user token
			$user_token = $this->db->get_where('user_token', ['token' => $token])->row_array();

			//cek token
			if ($user_token) {

				//cek waktu token apakah sebelum 24 jam
	...
	public function changePassword()
	{
		...
		$this->form_validation->set_rules('password1', 'Password', 'trim|required|min_length[3]|matches[password3]');
		$this->form_validation->set_rules('password3', 'Repeat Password', 'trim|required|min_length[3]|matches[password1]');
